...
- **`lambda_release` (module self-downloads its own release) over a CI pre-apply hook** (e.g. Spacelift/Terrateam/etc `before_plan` shelling out to `curl`) for getting code onto non-interactive runners: a hook lives entirely outside this repo's Terraform, in whatever pipeline-specific config format the consumer's CI platform uses, so it can't be versioned, tested, or discovered alongside the module itself, and its pinned release URL has no structural link to the module's own `source` ref — nothing stops the two from silently drifting apart. A hook was also observed in practice to hide real code changes: bumping only the module's `source` ref, with no local build and no CI-staged zip, changes nothing about `source_code_hash` (Section 4's data-source fallback returns whatever's already deployed), so a CI plan can report "no changes" even though the Lambda's code did change. `lambda_release` fixes this by making the release itself a real, typed Terraform input living in the same module block as `source`, rather than a raw string buried in separate CI-platform config — this doesn't eliminate the need to manually keep `lambda_release.version` and `source`'s ref in sync (Terraform's `source` argument can't reference a variable at all, so no mechanism can fully automate this — see GUD-008), but same-block proximity makes drift far easier to notice than a hook living in an entirely different file and config format. The cost is adding `hashicorp/http`/`hashicorp/local` as provider dependencies and the state-size/plan-time cost noted in PLT-003.
...
- **PLT-003**: `hashicorp/http` (>= 3.3.0, for binary-safe `response_body_base64`) and `hashicorp/local` (>= 2.0.0, for `local_file`'s `content_base64sha256`) — the module's only non-`aws` provider dependencies. `required_providers` is a static declaration Terraform resolves at `terraform init` time, independent of the `count` gating their actual usage (GUD-008) — every consumer downloads and locks both providers regardless of whether `lambda_release.version` is ever set, not just the ones who use it. The larger, genuinely conditional cost applies whenever tier (1) or tier (2) (GUD-008) is actually active — a real local build at `lambda_zip_path`, or `lambda_release.version` set — and splits into two distinct effects rather than one: whichever tier is active mirrors its zip's bytes into a `local_file` (GUD-008), growing Terraform state size by roughly the zip's size, and that stays roughly flat afterward — state holds a current snapshot, not a history, so new bytes replace the stored value rather than adding a second copy alongside it. What recurs on every single `plan`/`apply` where `lambda_release.version` specifically is set, even when it hasn't changed, is a real network fetch and a full rewrite of that state entry, since Terraform data sources re-fetch on every plan rather than caching — the recurring cost there is plan time/latency, not ever-growing state size. A local build via `lambda_zip_path` doesn't re-fetch anything (it's a plan-time file read, not a data source), so it only recurs when the local file's own content actually changes. A consumer with a versioned state backend (e.g. S3 bucket versioning) is the exception where storage does genuinely accumulate across applies, since each apply's full-state snapshot bakes in that zip-sized entry even though the live state file itself stays flat. Expect a `Warning: Response body is not recognized as UTF-8` on every `plan`/`apply` where `lambda_release.version` is set — harmless, not a correctness signal. `hashicorp/http` always populates its plain-string `response_body` attribute internally and warns when it isn't valid UTF-8 regardless of which attribute is actually read; this module only ever reads `response_body_base64` (binary-safe), so the warning doesn't reflect any problem with the downloaded content, confirmed by downloading a real release through this exact mechanism and matching GitHub's own reported digest bit-for-bit.
