...
- **PLT-003**: `hashicorp/http` (>= 3.3.0, for binary-safe `response_body_base64`) and `hashicorp/local` (>= 2.0.0, for `local_file`'s `content_base64sha256`) — the module's only non-`aws` provider dependencies. `required_providers` is a static declaration Terraform resolves at `terraform init` time, independent of the `count` gating their actual usage (GUD-008) — every consumer downloads and locks both providers regardless of whether `lambda_release.version` is ever set, not just the ones who use it. The larger, genuinely conditional cost only applies when `lambda_release.version` is actually set, and splits into two distinct effects rather than one: turning it on grows Terraform state size by roughly the zip's size once, and that stays roughly flat afterward — state holds a current snapshot, not a history, so a version bump replaces the stored value rather than adding a second copy alongside it. What recurs on every single `plan`/`apply` where `version` is set, even when it hasn't changed, is a real network fetch and a full rewrite of that state entry, since Terraform data sources re-fetch on every plan rather than caching — the recurring cost is plan time/latency, not ever-growing state size. A consumer with a versioned state backend (e.g. S3 bucket versioning) is the exception where storage does genuinely accumulate across applies, since each apply's full-state snapshot bakes in that zip-sized entry even though the live state file itself stays flat.
