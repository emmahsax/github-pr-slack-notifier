...
# var.lambda_zip_path (an arbitrary, machine-specific path) or a
...
# NOT because of `lifecycle.ignore_changes`. An earlier version of this
# module used `ignore_changes = [filename]` to paper over filename's literal
# value differing across machines, but that freezes filename to whatever
# was true at the moment it was first set, permanently — including once
# that value stops being a real, readable file. Every subsequent apply then
# tries to re-read that stale path and fails, even when a real source
# (local build or lambda_release) is genuinely available. Routing every
# tier through this one fixed path avoids the problem at its root: a tier-3
# apply (no local build, no lambda_release.version) computes this exact
# same path with no local_file behind it and a source_code_hash matching
# what's already deployed, so it never triggers a real read, whether or not
...
# (var.lambda_zip_path) to lambda_deploy_zip_path, so it's indistinguishable
# from a tier 2 release download by the time aws_lambda_function sees it.
resource "local_file" "lambda_zip_copy" {
  count = local.lambda_zip_exists ? 1 : 0

  filename       = local.lambda_deploy_zip_path
  content_base64 = filebase64(var.lambda_zip_path)
}

# Tier 2: only fetched when var.lambda_release.version is set AND no local
# build exists at var.lambda_zip_path. Downloads the tagged GitHub Release's
